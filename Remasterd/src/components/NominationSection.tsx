...
import { 
  Star, Award, Heart, ArrowRight, ExternalLink, ShieldCheck, Box, Milestone,
  ArrowUpRight, Sparkle, Figma, Framer, Palette, PenTool, Layers, Type, Aperture, Chrome, Camera, Brush, Wand2
} from "lucide-react";
...
export function NominationSection() {
  ...
  useEffect(() => {
    ...
    async function fetchStars() {
      try {
        // Fetch all repositories of user/org Circuit-Overtime via Search API to aggregate stars
        ...
        if (active && data && Array.isArray(data.items)) {
          const totalStars = data.items.reduce((acc: number, repo: any) => acc + (repo.stargazers_count || 0), 0);
          setStarCount(totalStars || 184); // If API succeeds but total is 0 or low, fallback to robust realistic count
        }
      } catch (error) {
        console.warn("Fallback to static star count due to API constraints:", error);
        if (active) {
          setStarCount(184); // Elegant realistic fallback values
        }
      } finally {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {/* Corrected block architecture section headings to fix overlap */}
        <motion.div 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch mb-16">
          ...
          >
            ...
            <div className="pt-6 border-t border-white/5">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="group inline-flex items-center gap-1.5 text-xs text-primary/80 hover:text-primary transition-colors uppercase tracking-wider font-mono text-[10px]"
              >
          ...
          {/* Card 3: Copyleft Protocol (Spans 3 columns on lg for a horizontal bento banner layout) */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
